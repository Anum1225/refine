...
export function simplifyBoltActions(input: string): string {
  // Using regex to match refineAction tags that have type="file"
  const regex = /(<refineAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/refineAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<refineAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</refineAction>`;
    });

  return `<refineArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</refineArtifact>`;
}
