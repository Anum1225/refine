    ...
    <Card className="w-full max-w-md p-6">
      <div className="text-center mb-6">
        ...
        <p className="text-bolt-elements-textSecondary mt-2">
          {resetMode 
            ...
            : 'Sign in to your Refine.new account'
          }
        </p>
      </div>
      ...
      <div className="mt-6 text-center">
        <p className="text-sm text-bolt-elements-textSecondary">
          {resetMode ? 'Remember your password?' : "Don't have an account?"}{' '}
          <button
            type="button"
            onClick={resetMode ? () => setResetMode(false) : onToggleMode}
            className="text-accent-600 hover:text-accent-700 font-medium transition-colors"
          >
            {resetMode ? 'Sign in' : 'Sign up'}
          </button>
        </p>
      </div>
    </Card>
